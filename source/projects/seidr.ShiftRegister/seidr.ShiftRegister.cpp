...
ShiftRegister_MAX::ShiftRegister_MAX(const atoms &args) {
    ...
    for (int i = 0; i < number_of_outputs; i++) {
        outputs.push_back(std::make_unique<outlet<>>(
            this, "( int | bang ) output " + std::to_string(i)));
    }
...
void ShiftRegister_MAX::handle_outputs() {
    ...
    for (int i = 0; i < outputs.size() - 1; i++) {
        this->outputs[i]->send(
            this->send_bangs ? bang() : atoms({(uint64_t)this->sr.get(i)}));
    }
}
...
void ShiftRegister_MAX::handle_through() {
    ...
    if (every_output || dt != last_output[8].get()) {
        this->outputs[last_output_index]->send(
            this->send_bangs ? bang() : c74::min::atoms({dt}));
...
int ShiftRegister_MAX::size() { return this->sr.size(); }

int ShiftRegister_MAX::step() { return this->sr.step(); }

uint64_t ShiftRegister_MAX::get(int i) { return this->sr.get(i); }
